...
def init_passage_db():
    ...
    cur.execute("""CREATE TABLE IF NOT EXISTS embeddings (
        passage_id INTEGER NOT NULL, model TEXT NOT NULL, vector BLOB NOT NULL,
...
def save_embedding(passage_id: int, model: str, vector: np.ndarray):
    buf = io.BytesIO(); np.save(buf, vector)
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    cur.execute("""INSERT OR REPLACE INTO embeddings (passage_id, model, vector)
                   VALUES (?, ?, ?)""", (passage_id, model, buf.getvalue()))
    conn.commit(); conn.close()
